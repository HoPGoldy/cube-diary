...
const getNewMenuPos = (
   ...
   */
  const top =
    screenHeight > prevRect.top + menuTotalHeight
      ? prevRect.top
      : Math.max(screenHeight - menuTotalHeight - 10, 0);
  /**
   * 看看屏幕右边还够不够容纳一个菜单的宽度了
   * 如果够就往右展开，否则就往左展开
   */
  const left =
    screenWidth > prevRect.right + MENU_WIDTH
      ? prevRect.right + offset
      : prevRect.left - MENU_WIDTH - offset;
...
export const TreeMenu: FC<PropsWithChildren<Props>> = (props) => {
  ...
  /** 关闭全部菜单的防抖 */
  const closeAllThrottle = useMemo(
    () => debounce(() => setMenuLists([]), 200),
    [],
  );
  ...
  return (
    <>
      ...
      <TransitionGroup>{menuLists.map(renderMenuLists)}</TransitionGroup>
    </>
  );
};
