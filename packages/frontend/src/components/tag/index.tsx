...
export const AddTag: FC<AddTagProps> = (props) => {
  ...
  const renderContent = () => {
    if (!editing) return <span onClick={() => setEditing(true)}>+ 新增</span>;
  ...
  return (
    <AntdTag className="cursor-pointer truncate">{renderContent()}</AntdTag>
  );
};
...
export const EditTagEntry: FC<EditTagEntryProps> = (props) => {
  return (
    <AntdTag className="cursor-pointer truncate" onClick={props.onClick}>
      {props.label}
    </AntdTag>
  );
};
